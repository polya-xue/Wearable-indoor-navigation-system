 ...
 * Debug Macros
 */
#if ((PLATFORM_TYPE == PLATFORM_WINDOWS) && (DEBUG))
...
{  \
	fprintf ((FILE*)file, "[%s]:[%d]:",__FILE__,__LINE__);  \
    fprintf ((FILE*)file, format ,##__VA_ARGS__);  \
    fflush ((FILE*)file);  \
}
...
{  \
    fprintf (file, "[%s]:[%d]:[--INFO--]: ", __FILE__, __LINE__);  \
...
{  \
    fprintf (file, "[%s]:[%d]:[--ERR--]: ", __FILE__, __LINE__);  \
    fprintf (file, format ,##__VA_ARGS__);  \
    fflush (file);  \
}
...
#elif ((PLATFORM_TYPE == PLATFORM_ARM) && (DEBUG))
#include <stdio.h>
